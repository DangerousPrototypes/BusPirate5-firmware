...
| ~~`src/ui/ui_cmdln.c`~~ | ~~`cmdln_consume_white_space()`~~ | ~~No~~ **static** ✅ | **DONE** — marked `static` |
| ~~`src/ui/ui_cmdln.c`~~ | ~~`cmdln_args_get_string()`~~ | ~~No~~ **static** ✅ | **DONE** — marked `static` |
| ~~`src/ui/ui_cmdln.c`~~ | ~~`cmdln_args_get_hex()`~~ | ~~No~~ **static** ✅ | **DONE** — marked `static` |
| ~~`src/ui/ui_cmdln.c`~~ | ~~`cmdln_args_get_bin()`~~ | ~~No~~ **static** ✅ | **DONE** — marked `static` |
| ~~`src/ui/ui_cmdln.c`~~ | ~~`cmdln_args_get_dec()`~~ | ~~No~~ **static** ✅ | **DONE** — marked `static` |
| ~~`src/ui/ui_cmdln.c`~~ | ~~`cmdln_args_find_flag_internal()`~~ | ~~No~~ **static** ✅ | **DONE** — marked `static` |
...
|------|-------------|-------------|----------------|
| ~~`src/pirate/hw1wire_pio.c`~~ | ~~400 (6 blocks)~~ | ROM search test, DS18B20 demo, `onewire_temp_app()` — all moved to `commands/1wire/` | **DONE ✅** — 406 lines removed |
| ~~`src/commands/global/flat.c`~~ | ~~130 (4 blocks)~~ | FlatBuffer test/debug functions — file removed entirely | **DONE ✅** — file deleted |
| ~~`src/commands/i2c/ddr4.c`~~ | ~~50 (4 blocks)~~ | Polling idle status, DDR5 SPD reading logic (wrong file) | **DONE ✅** — 45 lines removed |
| ~~`src/commands/i2c/usbpdo.c`~~ | ~~90 (3 blocks)~~ | Alternative `fusb_read_source_capabilities()`, PDO request error handling | **DONE ✅** — 98 lines removed |
...
| ~~`src/lib/sigrok/pico_sdk_sigrok.c` (1,387 lines)~~ | — | — | **DELETED ✅** |
| ~~`src/display/background_image_v4-orig.h`~~ | — | — | **DELETED ✅** |
| ~~`src/display/robot16.h`~~ | — | — | **DELETED ✅** |
| ~~`src/deprecated/*.c` (12 files)~~ | — | — | **DELETED ✅** — entire folder removed |
...
|------------|------------|------------|----------------|
| ~~`src/ui/ui_parse.c` (all 16 functions)~~ | ~~`src/deprecated/ui_parse.c` (all 16 functions)~~ | — | **DONE ✅** — deprecated folder deleted |
| ~~`src/ui/ui_format.c` (5 functions)~~ | ~~`src/deprecated/ui_format.c` (5 matching + 1 extra)~~ | — | **DONE ✅** — deprecated folder deleted |
| ~~`src/deprecated/bp_args.c`~~ | `src/lib/bp_args/bp_cmd.c` | — | **DONE ✅** — deprecated folder deleted |
...
**✅ DELETED** — `src/deprecated/` (12 files, ~4,000 lines) removed on 2026-02-21.
...
1. ~~**Delete `src/deprecated/` entirely** (~4,000 lines)~~ ✅ **DONE** — ~4,000 lines removed
2. ~~**Remove `#if 0` blocks in `src/pirate/hw1wire_pio.c`** (~400 lines)~~ ✅ **DONE** — 406 lines removed
3. ~~**Remove `#if 0` blocks in `src/commands/global/flat.c`** (~130 lines)~~ ✅ **DONE** — file deleted entirely
4. ~~**Remove `#if 0` blocks in `src/commands/i2c/usbpdo.c`** (~90 lines)~~ ✅ **DONE** — 98 lines removed
5. ~~**Remove `#if 0` blocks in `src/commands/i2c/ddr4.c`** (~50 lines)~~ ✅ **DONE** — 45 lines removed
6. ~~**Delete `src/display/background_image_v4-orig.h`**~~ ✅ **DONE**
7. ~~**Delete `src/display/robot16.h`**~~ ✅ **DONE**
8. ~~**Remove `bio_buf_test()`** from `src/pirate/bio.c` and its declaration from `bio.h`~~ ✅ **DONE**
9. ~~**Remove `rx_uart_disable()`** from `src/debug_uart.c`~~ ✅ **DONE**
10. ~~**Delete `src/lib/sigrok/pico_sdk_sigrok.c`** (1,387 lines)~~ ✅ **DONE**
...
1. ~~**Make internal helpers static in `ui_cmdln.c`**: 6 functions (`cmdln_consume_white_space`, `cmdln_args_get_string/hex/bin/dec`, `cmdln_args_find_flag_internal`) are only used within the file — mark them `static`~~ ✅ **DONE**
...
| ~~`src/lib/sigrok/pico_sdk_sigrok.c`~~ | — | **DELETED ✅** |
...
|----------|--------|--------|--------|-------------|
| ~~1~~ | ~~Delete `src/deprecated/` folder~~ | — | — | ✅ ~4,000 |
| ~~2~~ | ~~Remove `#if 0` blocks in `hw1wire_pio.c`~~ | — | — | ✅ ~406 |
| ~~3~~ | ~~Remove `#if 0` blocks in `flat.c`, `usbpdo.c`, `ddr4.c`~~ | — | — | ✅ ~268 |
| ~~4~~ | ~~Delete orphaned display headers (`robot16.h`, `background_image_v4-orig.h`)~~ | — | — | ✅ ~50 |
| ~~5~~ | ~~Delete orphaned `src/lib/sigrok/pico_sdk_sigrok.c`~~ | — | — | ✅ ~1,387 |
| ~~6~~ | ~~Remove dead functions (`bio_buf_test`, `rx_uart_disable`)~~ | — | — | ✅ ~20 |
| ~~7~~ | ~~Make `ui_cmdln.c` internal helpers `static`~~ | — | — | ✅ 0 (quality) |
...
**✅ ~6,100 lines removed.** Remaining estimated removable: **~2,000 lines**.
