...
void cmd_selftest(void){
    ...
    //USB/VOLTAGE/ADC/AMUX test: read the (USB) power supply
    if(selftest_adc()) fails++;

    //TF flash card check: was TF flash card detected?
    if(selftest_flash()) fails++;

    //psu test
    if(selftest_psu(3.3, 100, true)) fails++;

    //detect REV10 failure of op-amp
    if(selftest_psu_opamp()) fails++;

    // BIO float test
    if(selftest_bio_float()) fails++;

    // BIO high test
    if(selftest_bio_high()) fails++;
    ...
    if(selftest_pullup_low()) fails++;

    //PSU test with current override
    if(selftest_current_override()) fails++;
 
    //PSU test with current limit
    if(selftest_current_limit()) fails++;
