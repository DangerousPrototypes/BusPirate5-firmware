...
typedef struct __attribute__((packed)) {
    ...
    ddr4_nvm_jedec_sdram_info_t first_sdram; // First SDRAM Info (bytes 4-7)
    ddr4_nvm_jedec_sdram_info_t second_sdram; // Second SDRAM Info (bytes 8-11)
} ddr4_nvm_jedec_info_t;
...
bool ddr4_set_legacy_page(uint8_t page){
...
bool ddr4_detect_spd_quick(void){
    uint8_t data[2];
    ...
    ddr4_spd_volatile_t* spd = (ddr4_spd_volatile_t*)data;
...
bool ddr4_read_pages_128bytes(bool eeprom, uint8_t start_page, uint8_t num_pages, uint8_t* data) {
    if(!eeprom){
        data[0]=DDR5_SPD_ACCESS_REG;//read volatile memory, start at 0x00
    }else{
        data[0]=DDR5_SPD_ACCESS_NVM;//read EEPROM page start at 0x00 
        if(ddr4_set_legacy_page(start_page)) return true; //set the page for the legacy mode
    }
...
static const char *ddr4_decode_jep106_jedec_id(uint8_t b0, uint8_t b1){
    int bank=(b0 & 0xf);
...
uint16_t ddr4_crc16(const uint8_t *spd, uint32_t cnt) {
    if (spd == NULL) {
...
void ddr4_protection_block_table(uint8_t *old_data, uint8_t *verify_data, bool show_old) {
    // lock table header
    printf("Block   |");
...
*
*/
float ddr4_get_temperature(uint8_t t_high, uint8_t t_low, uint8_t t_resolution) {
...
bool ddr4_decode_volatile_memory(uint8_t* data) {
    
    ddr4_spd_volatile_t* spd = (ddr4_spd_volatile_t*)data;

    printf("Device Revision: %d.%d\r\n", (spd->device_revision_major+1), spd->device_revision_minor);
    printf("Vendor ID: 0x%02X%02X (%s)\r\n", spd->vendor_id_byte0, spd->vendor_id_byte1, ddr4_decode_jep106_jedec_id(spd->vendor_id_byte0, spd->vendor_id_byte1));
    ...
    ddr4_protection_block_table((uint8_t[]){0x00,0x00}, (uint8_t[]){spd->write_protection_nvm_blocks_low, spd->write_protection_nvm_blocks_high}, false); 
    #if 0
    for(uint8_t i=0; i<8; i++){
    ...
    printf("TS Current Sensed Temperature: 0x%02X%02X (%2.1fc)\r\n", 
        ...
        ddr4_get_temperature(spd->ts_current_sensed_temperature_high, spd->ts_current_sensed_temperature_low, spd->ts_resolution));
    printf("TS Temperature High Limit: 0x%02X%02X (%2.1fc)\r\n", 
        spd->ts_temp_high_limit_high, 
        spd->ts_temp_high_limit_low, 
        ddr4_get_temperature(spd->ts_temp_high_limit_high, spd->ts_temp_high_limit_low, spd->ts_resolution));
    printf("TS Temperature Low Limit: 0x%02X%02X (%2.1fc)\r\n", 
        spd->ts_temp_low_limit_high, 
        spd->ts_temp_low_limit_low, 
        ddr4_get_temperature(spd->ts_temp_low_limit_high, spd->ts_temp_low_limit_low, spd->ts_resolution));
    printf("TS Critical Temperature High Limit: 0x%02X%02X (%2.1fc)\r\n", spd->ts_critical_temp_high_limit_high, spd->ts_critical_temp_high_limit_low, 
        ddr4_get_temperature(spd->ts_critical_temp_high_limit_high, spd->ts_critical_temp_high_limit_low, spd->ts_resolution)); 
    printf("TS Critical Temperature Low Limit: 0x%02X%02X (%2.1fc)\r\n", 
        ...
        ddr4_get_temperature(spd->ts_critical_temp_low_limit_high, spd->ts_critical_temp_low_limit_low, spd->ts_resolution));
...
bool ddr4_nvm_jedec_crc(uint8_t *data){
    printf("CRC verify\r\nStored CRC (bytes 510:511): 0x%02X 0x%02X\r\n", data[510], data[511]);
    uint16_t crc= ddr4_crc16(data, 510);
...
// 0 if ok, 1 if error
bool ddr4_sdram_info(ddr4_nvm_jedec_sdram_info_t *sdram_info) {   
    ...
    if(sdram_info->density_per_die <= 0b1000) {
        ...
        ddr4_print_unknown(sdram_info->density_per_die);
    }
    ...
    if(sdram_info->dies_per_package <= 0b101) {
        ...
        ddr4_print_unknown(sdram_info->dies_per_package);
    }
    ...
    if(sdram_info->io_width <=0b11) {
        ...
        ddr4_print_unknown(sdram_info->io_width);
    }
    ...
    if(sdram_info->bank_groups <= 0b11) {
        ...
        ddr4_print_unknown(sdram_info->bank_groups);
    }
    ...
    if(sdram_info->banks_per_group <= 0b010) {
        ...
        ddr4_print_unknown(sdram_info->banks_per_group);
    }

    return false;
}

bool ddr4_nvm_jedec_decode_data(uint8_t *data) {
    ...
    ddr4_nvm_jedec_info_t* spd_info = (ddr4_nvm_jedec_info_t*)data;
    //check the signature
    ...
    ddr4_sdram_info(&spd_info->first_sdram);
    printf("Second SDRAM: 0x%02X, 0x%02X, 0x%02X, 0x%02X\r\n", data[8], data[9], data[10], data[11]);
    ddr4_sdram_info(&spd_info->second_sdram);
    return false;
}

bool ddr4_nvm_jedec_decode_manuf(uint8_t *data){
    //use the struct to extract the manufacturing information
    ddr4_nvm_jedec_manuf_info_t* manuf_info = (ddr4_nvm_jedec_manuf_info_t*)data;

    printf("  Module Manuf. Code: 0x%02X%02X (%s)\r\n", manuf_info->manuf_code[0], manuf_info->manuf_code[1], ddr4_decode_jep106_jedec_id(manuf_info->manuf_code[0], manuf_info->manuf_code[1]));
    ...
    printf("  DRAM Manuf. Code: 0x%02X%02X (%s)\r\n", manuf_info->dram_manuf_code[0], manuf_info->dram_manuf_code[1], ddr4_decode_jep106_jedec_id(manuf_info->dram_manuf_code[0], manuf_info->dram_manuf_code[1]));
    printf("  DRAM Stepping: 0x%02X\r\n", manuf_info->dram_stepping);
    return false;
}

//search in EEPROM bits of data, with up to X=10 trailing 0s
void ddr4_search_upa(uint8_t *data, uint32_t start, uint32_t end) {
...
bool ddr4_nvm_search(uint8_t *data) {
    printf("\r\nSearching Manuf. Specific Data area block 9:");
    ddr4_search_upa(data, 0x22B, 0x27F); //search in 0x22B-0x27F
    printf("\r\n\r\nSearching End User Programmable Area blocks 10-15:");
    ddr4_search_upa(data, 0x280, 0x3FF); //search in 0x280-0x3FF
    printf("\r\n\r\n");
    return false;
}
...
bool ddr4_probe(uint8_t *buffer) {
    ...
    // cast it to the ddr4_spd_volatile_t structure
    //detect if spd present
    if(ddr4_detect_spd_quick()) return true; //check if the device is DDR5 SPD

    if(ddr4_read_pages_128bytes(false, 0, 1, buffer)) return true; //read volatile memory, start at 0x00
    
    ddr4_decode_volatile_memory(buffer); //decode the volatile memory

    //read 1024 bytes from the DDR5 SPD NVM
    if(ddr4_read_pages_128bytes(true, 0, 8, buffer)) return true; //read EEPROM page 0-7, start at 0x00

    printf("\r\nSPD EEPROM JEDEC Data blocks 0-7:\r\n");
    //if(ddr4_read_pages_128bytes(true, 0, 4, buffer)) return true; //read EEPROM page 0-4, start at 0x00
    if(ddr4_nvm_jedec_crc(buffer)) return true; //check CRC of the first 512 bytes
    if(ddr4_nvm_jedec_decode_data(buffer)) return true; //decode the first 512 bytes of the EEPROM

    printf("\r\nSPD EEPROM JEDEC Manufacturing Information blocks 8-9:\r\n");
    //if(ddr4_read_pages_128bytes(true, 0b100, 4, buffer)) return true; //read EEPROM page 5-8, start at 0x00
    if(ddr4_nvm_jedec_decode_manuf(&buffer[0x200])) return true; //decode the manufacturing information
    if(ddr4_nvm_search(buffer)) return true; //search for the end user programmable area
    return false;
}
...
bool ddr4_lock_bits_write_verify(uint8_t lb0, uint8_t lb1) {
...
bool ddr4_verify(FIL *file_handle, uint8_t *buffer) {
    ...
    for(uint32_t i=0; i<DDR5_SPD_SIZE/128; i++){
        ...
        if(ddr4_read_pages_128bytes(true, i, 1, verify_buffer)){
            file_close(file_handle); // close the file
            return true;
        }
...
bool ddr4_write_from_file(FIL *file_handle, uint8_t *buffer) {
    ...
    if(f_rewind(file_handle)) { // rewind the file to the beginning
        ...
        goto ddr4_write_error; 
    }
    if(ddr4_nvm_jedec_crc(buffer)) return true; // check CRC of the first 512 bytes

    //detect if spd present
    if(ddr4_detect_spd_quick()) goto ddr4_write_error; //check if the device is DDR5 SPD

    //check if write enabled (HSA pin grounded)
    if(i2c_transaction(DDR5_SPD_I2C_WRITE_ADDR, (uint8_t[]){DDR5_SPD_MR48}, 1u, buffer, 1)) goto ddr4_write_error; // read the Device Status Register (MR48)
    ...
    if(buffer[0] & DDR5_SPD_MR48_OVERRIDE_STATUP) {
        ...
        goto ddr4_write_error; // if not, we cannot write to the NVM
    }
    ...
    if(i2c_transaction(DDR5_SPD_I2C_WRITE_ADDR, (uint8_t[]){DDR5_SPD_MR12}, 1u, original_lock_bits, 2)) {
        goto ddr4_write_error; // read the NVM block lock bits
    }
    printf("Saving NVM block lock bits: 0x%02X 0x%02X\r\n", original_lock_bits[0], original_lock_bits[1]);

    // unlock the block lock bits
    if(ddr4_lock_bits_write_verify(0x00, 0x00)) {
        goto ddr4_write_error; // if the lock bits were not written successfully
    }
    ...
    for(uint8_t i=0; i<8; i++){
        ...
        if(ddr4_set_legacy_page(i)) goto ddr4_write_error; // set the page for the legacy mode  
        ...
        for(uint8_t j=0; j<128/16; j++){
            ...
            // wait for the write to complete
            if(ddr4_wait_idle()) return true; // wait until the write operation is complete
        }
    }
    printf(" Done!\r\n");

    //restore NVM lock bits
    if(ddr4_lock_bits_write_verify(original_lock_bits[0], original_lock_bits[1])) {
        goto ddr4_write_error; // if the lock bits were not written successfully
    }
    ...
    if(ddr4_verify(file_handle, buffer)){
        ...
        return true; // on fail file is closed in ddr4_verify
    }else{
        printf("Verify: OK\r\n"); //on success file is closed in ddr4_verify
    }
...
bool ddr4_crc_file(FIL *file_handle, char *buffer){
    //get file size
    ...
    return ddr4_nvm_jedec_crc(buffer); 
}
...
bool ddr4_lock(uint8_t block, bool lock, bool update) {
    uint8_t old_data[2];
    ...
    ddr4_protection_block_table(old_data, verify_data, true); // print the lock bits table
    return false;
}
...
void ddr4_handler(struct command_result* res) {
    ...
    switch(action) {
        ...
        case DDR5_LOCK:
            ...
            ddr4_lock(block_flag, true, lock_update);
            break;
        case DDR5_UNLOCK:
            if(lock_update) printf("Unlocking NVM block %d\r\n", block_flag);
            ddr4_lock(block_flag, false, lock_update);
            break;
        case DDR5_CRC:
            printf("Checking CRC for JEDEC blocks 0-7, file: %s\r\n", file);
            ddr4_crc_file(&file_handle, buffer);
            break;
        case DDR5_PATCH:
            printf("Checking CRC for JEDEC blocks 0-7, file: %s\r\n", file);
            //get file size
            ...
            // check the CRC of the first 512 bytes
            uint16_t crc= ddr4_crc16(buffer, 510);
            printf("Calculated CRC: 0x%02X 0x%02X\r\n", crc&0xff, crc >> 8);
