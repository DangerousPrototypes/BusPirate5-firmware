...
void sniff_handler(struct command_result *res){
    ...
    spisnif_program_init(pio, pio_state_machine, pio_loaded_offset, bio2bufiopin[BIO3], bio2bufiopin[BIO0], bio2bufiopin[BIO1]);

    static const char pin_labels[][5] = {"DAT0","DAT1","SCLK","SCS"}; 
    
    system_bio_claim(true, BIO0, BP_PIN_MODE, pin_labels[0]);
    system_bio_claim(true, BIO1, BP_PIN_MODE, pin_labels[2]);
