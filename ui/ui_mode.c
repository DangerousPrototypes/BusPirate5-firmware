...
{
    printf(" %sCurrent setting: %s%s\r\n", ui_term_color_info(),ui_const_display_formats[system_config.display_format], ui_term_color_reset());
	for(uint8_t i=0; i<(*menu).menu_items_count; i++)
	{	
...
void ui_mode_int_display_format(opt_args (*args), struct command_result *res)
{
	...
	if( result.error || result.no_value || result.exit || ((mode)>count_of(ui_const_display_formats)) || ((mode)==0) )
	{
	...
	if(error)			// no integer found
	{
		...
		static const struct ui_prompt mode_menu={
			...
			0,
			count_of(ui_const_display_formats),
			T_MODE_MODE, 
			0,0,0,
			0,
			&cfg
