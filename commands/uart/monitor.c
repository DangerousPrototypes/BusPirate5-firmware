...
void uart_monitor_handler(struct command_result *res){
    ...
    busy_wait_ms(10);
    if(uart_is_readable(uart0)) (void)uart_getc(uart0);
    if(uart_is_readable(uart1)) (void)uart_getc(uart1);
...
void monitor_plank_test(void){
    ...
    //flush fifos
    while(uart_is_readable(UART_DTE)) uart_getc(UART_DTE);
    while(uart_is_readable(UART_DCE)) uart_getc(UART_DCE);
    //DTE to DCE
    printf("DTE TX:");
