...
# Overall Process:
#
# 1. `en-us.h` header file is parsed into a dictionary
#    e.g.,  `[T_HELP_FOO]="Bar"` line uses string key `T_HELP_FOO` with string value `Bar`
# 2. That dictionary is canonicalized, replacing the valueString with an object of form:
#    `Localized`: valueString
#    `EN_US`: valueString
#    `Comments`: "Autogenerated from en-us.h"
#    `DataTypes`: format specifiers extracted from the valueString
# 3. Current en-us is then checked against historical version
#    * Did format specifiers change?
#    * Did string have other changes (because could invalidate translations)?
#    * If so, this will cause an error ... to force review, hopefully preventing
#      accidental changes that would cause corruption.
# 4. Each translation file (./templates/*.json) is then validated against the current
#    en-us dictionary.  Checks include:
#    * No localized string provided? --> set to use en-us string (stores NULL)
#    * Translated string is identical to en-us? --> set to use en-us string (stores NULL)
#    * Translated string format specifiers do NOT match en-us --> set to use en-us string
#    * Translation was based on a different en-us string --> detected, but currently allowed
#    * Translation was for a non-translatable string --> set to use en-us string (stores NULL)
# 5. Creation of the localized language header file (e.g., bs-ba.json --> bs-ba.h)
#
# Verbose output is available for step 4, see `DebugJsonConversion` for options.
#     
    ...
    for filename in os.listdir(json_directory_path):
        if filename.endswith('.json') and filename != 'en-us.json':
            ...
            with open(infile_path, 'r', encoding="utf8") as json_file:
                ...
                # Verify each of the translation's keys/values that would be used (exist in en-us)
                # Create same keys in the same order, to generate compatible .h file.
                for k in base_translation:
                    # Untranslated?  That's OK, move on.
                    base_obj = base_translation[k]
                    if not k in target_translation:
                        debug_untranslated.append(k)
                        output_translation[k] = None
                        continue
                    # Also untranslated. Same as above ... move on.
                    tgt_obj = target_translation[k]
                    if tgt_obj is None:
                        debug_untranslated.append(k)
                        output_translation[k] = None
                        continue
                    # `Localized` does not exist in JSON? Essentially the same as untranslated.
                    if tgt_obj['Localized'] is None:
                        debug_null.append(k)
                        output_translation[k] = None
                        continue
                    # Exact match of the current `en-us` string?  No need to store another copy in the firmware.
                    if base_obj['Localized'] == tgt_obj['Localized']:
                        ...
                        output_translation[k] = None
                        continue
                    # For any string in the target translation, ensure any embedded format strings
                    # match those used in the `en-us` string.
                    # Failure to check this can result in corruption / crashes (or worse).
                    ...
                    if diffs is not None:
                        ...
                        output_translation[k] = None
                        continue
                    # Translations keep track of the EN-US string they were translated from.
                    # Verify the translation was based on that same EN-US string.
                    if tgt_obj['EN_US'] is None or base_obj['Localized'] != tgt_obj['EN_US']:
                        debug_outdated_translation.append(k)
                        ## Because already checked format strings, it's probably safe to
                        ## allow the old translation, even though the en-us string has changed.
...
# Canonicalize the `en-us` autogenerated data
...
# Note: Check that format specifiers (if any) in the strings are unchanged.
#       Later improvements might allow compatible changes (not implemented).
...
# NOTE: If this errors out, then likely the string changes did not get updated
#       in the checked in `history/en-us.json` file.
#       This is critical, and used to ensure changes to embedded format specifiers
#       will cause build breaks and cause additional review of such changes.
print(f"\n======================================================================")
...
if diff_all is not None:
    ...
    print()
    print(f"WARNING: Exiting to prevent accidental divergence of translations.")
    sys.exit(1)
