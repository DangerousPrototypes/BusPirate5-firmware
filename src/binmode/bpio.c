...
static inline void send_packet(flatcc_builder_t *B) {
    ...
    header[1] = (len >> 8) & 0xFF; // MSB
        ...
        for(size_t i = 0; i < len; i++) { //110uS
            bin_tx_fifo_put(buf[i]);
        }
    #else   
        while(true){//60uS
            if (tud_cdc_n_write_available(CDC_INTF) >= 2) {
                tud_cdc_n_write(CDC_INTF, header, 2);
        ...
        while(bytes_remaining) { //80uS
            if (tud_cdc_n_write_available(CDC_INTF) >= 64) {
                tud_cdc_n_write(CDC_INTF, &buf[cnt], bytes_remaining>64? 64 : bytes_remaining);
                tud_cdc_n_write_flush(CDC_INTF);
                cnt += (bytes_remaining>64? 64 : bytes_remaining);
                bytes_remaining -= (bytes_remaining>64? 64 : bytes_remaining);
            }
        }
    #endif
...
uint32_t data_request(bpio_RequestPacket_table_t packet, flatcc_builder_t *B) {
    ...
    //time_start = time_us_32(); // Start timing
    ...
    //time_end = time_us_32(); // End timing
    
    //***************************TIME START
    //prepare DataResponse
    bpio_DataResponse_start(B);
    if (request.bytes_read > 0 && !error) {
        ...
        for (uint16_t i = 0; i < request.bytes_read; i++) {
            bpio_DataResponse_data_read_push(B, &request.data_buf[i]); 
        }
...
void dirtyproto_mode(void) {
    ...
    #else
        if (tud_cdc_n_available(CDC_INTF)>=2) {     
            tud_cdc_n_read(CDC_INTF, buf, 2);
            len = buf[0] | (buf[1] << 8); // Read the length from the first two bytes
            if(bpio_debug) printf("[BPIO] Received length: %d\r\n", len);
            if(len==0 || len>sizeof(buf)){ 
                if(bpio_debug) printf("[BPIO] Flatbuffer size error: %d\r\n", len);
                // Send an error response.
                error_response("Flatbuffer size error: Invalid length", B);
    ...
    switch (binmode_state) {
        ...
        case BINMODE_PACKET:
            //get len bytes
                ...
                for(uint32_t i=0; i<len; i++) { //250uS
                ...
                uint32_t bytes_remaining = len;
                uint32_t cnt = 0;
                uint32_t time_start = time_us_32(); // Start timing
                while(bytes_remaining) { //200uS
                    if (tud_cdc_n_available(CDC_INTF)) {
                        uint read_bytes = tud_cdc_n_read(CDC_INTF, &buf[cnt], bytes_remaining>64? 64 : bytes_remaining);
                        cnt += read_bytes;
                        bytes_remaining -= read_bytes;
                    }
                    if (time_us_32() - time_start > 500000) { // Timeout after 500ms
            ...
            int ret = bpio_RequestPacket_verify_as_root(buf, 500); // Verify the flatbuffer packet (this will throw an error if invalid)
            if(ret){
                if(bpio_debug){
                    printf("[BPIO] Error: Invalid flatbuffer packet received, verify returned %s\r\n", flatcc_verify_error_string(ret));
                }
                // Send an error response.
                error_response("Invalid flatbuffer packet received", B);
                binmode_state = BINMODE_IDLE; // Reset state on error
                return;
            }
