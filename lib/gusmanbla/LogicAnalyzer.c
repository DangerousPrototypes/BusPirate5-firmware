...
void sendResponse(char* response, bool toWiFi)
{
    #ifdef USE_CYGW_WIFI
    if(toWiFi)
    {
    ...
    /*for (int i = 0; i < length;)
    {
        ...
        if (n)
        {
            int n2 = (int)tud_cdc_n_write(1, response + i, (uint32_t)n);
            tud_task();
...
void cdc_transfer(unsigned char* data, int len)
{
    ...
    while(left > 0)
    {
        ...
        if(avail)
        {
            int transferred = (int) tud_cdc_n_write(1, data + pos, avail);
            tud_task();
            tud_cdc_n_write_flush(1);
...
bool processUSBInput(bool skipProcessing)
{
    ...
    char data;
    bool has_data=bin_rx_fifo_try_get(&data);
...
void gusmanb_la(void)
{
    ...
    //Configure led
    #if defined (GPIO_LED) || defined (WS2812_LED)
    INIT_LED();
    LED_ON();
    #endif
