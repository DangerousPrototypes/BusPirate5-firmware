...
static inline void send_packet(flatcc_builder_t *B) {
    ...
    //copy up to 62 bytes of the flatbuffer data into the first transfer
    uint32_t header_bonus_bytes = len > 62 ? 62 : len; // Ensure we don't exceed the header size
    memcpy(header + 2, buf, header_bonus_bytes);
    
    //send header and first 62 bytes of flatbuffer data
    while(true){//60uS
        if (tud_cdc_n_write_available(CDC_INTF) >= 64) {
            tud_cdc_n_write(CDC_INTF, header, 2 + header_bonus_bytes);
            tud_cdc_n_write_flush(CDC_INTF);
            break;
        }        
    }
    
    // Send any remaining flatbuffer data with pointer arithmetic
    uint8_t *buf_ptr = buf;
    buf_ptr += header_bonus_bytes; // Move pointer past the header bonus bytes
    uint32_t bytes_remaining = len-header_bonus_bytes;
    while (bytes_remaining) { //80uS
        if (tud_cdc_n_write_available(CDC_INTF) >= 64) {
            uint32_t chunk_size = (bytes_remaining > 64) ? 64 : bytes_remaining;
...
uint32_t data_request(bpio_RequestPacket_table_t packet, flatcc_builder_t *B) {
    ...
    if (data_write_len > 0) {
        //TODO: can we instead send 1 byte to a protocol write handler, split all the steps up here?
        // prevents the need for a copy buffer....
...
void dirtyproto_mode(void) {
    ...
    if(len==0 || len>sizeof(buf)){ 
        error_response("Flatbuffer length invalid", B);
        return;
    }        
    ...
    while (bytes_remaining) {
        if (tud_cdc_n_available(CDC_INTF)) {
            uint32_t chunk_size = (bytes_remaining > 64) ? 64 : bytes_remaining;
            uint32_t read_bytes = tud_cdc_n_read(CDC_INTF, buf_ptr, chunk_size);
            buf_ptr += read_bytes;
