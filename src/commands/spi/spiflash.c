...
static inline uint32_t spiflash_next_count(uint32_t current_address, uint32_t end_address, uint32_t buff_cnt) {
...
bool flash_read_remsid(uint8_t* remsid_manuf, uint8_t* remsid_dev) {
    ...
    *remsid_dev = hwspi_read();
    hwspi_deselect();
    if (*remsid_manuf == 0x00 ||
        *remsid_manuf == 0xff) { // TODO: manuf ID has a checksum bit or something, list of man and dev ids?    {
        return false;
    }
    return true;
}
...
void spiflash_probe(void) {
    ...
    printf("\r\nREMSID (0x90)\t");
    if (has_remsid) {
        printf("0x%02x\t\t0x%02x", remsid_dev, remsid_manuf);
    } else {
        printf("--\t\t--");
    }
    printf("\r\nRDID (0x9F)\t");
    if (has_rdid) {
        printf("\t\t0x%02x\t\t0x%02x\t\t0x%02x", rdid_manuf, rdid_type, rdid_capacity);
    } else {
        printf("\t\t--\t\t--\t\t--");
    }
...
#define PTP_JEDEC 0
        ...
        printf("Table %d\t\t%s (0x%02x)\t%d.%d\t%d\t0x%06x\r\n",
               i,
               ptp_id == PTP_JEDEC ? ptp_jedec : ptp_manuf,
               ptp_id,
               ptp_rec->revision_major,
               ptp_rec->revision_minor,
               ptp_length,
               ptp_address);
        ...
        switch (ptp_id) {
            case PTP_JEDEC:
                ...
                printf("\r\nWrite granularity:");
                if (!ptp_j->write_granularity) {
                    printf("1B");
                } else {
                    printf(">=64B");
                }
                printf("\r\nWrite Enable Volatile: %d\r\nWrite Enable instruction: 0x%02x\r\n",
                       ptp_j->volatile_status_register_block_protect_bits,
                       (ptp_j->volatile_status_register_write_enable_instruction_select) ? 0x6 : 0x50);
                printf("4K erase instruction: ");
                if (ptp_j->erase_size == 0b01) {
                    printf("0x%02x", ptp_j->erase_instruction_4k);
                } else {
                    printf("--");
                }
                ...
                if (ptp_j->fast_read_112) {
                    printf("\t0x%02x", ptp_j->fast_read_112_read_instruction);
                } else {
                    printf("\t--");
                }
                if (ptp_j->fast_read_114) {
                    printf("\t0x%02x", ptp_j->fast_read_114_read_instruction);
                } else {
                    printf("\t--");
                }
                if (ptp_j->fast_read_122) {
                    printf("\t0x%02x", ptp_j->fast_read_122_read_instruction);
                } else {
                    printf("\t--");
                }
                if (ptp_j->fast_read_144) {
                    printf("\t0x%02x", ptp_j->fast_read_144_read_instruction);
                } else {
                    printf("\t--");
                }
                if (ptp_j->fast_read_222) {
                    printf("\t0x%02x", ptp_j->fast_read_222_read_instruction);
                } else {
                    printf("\t--");
                }
                if (ptp_j->fast_read_444) {
                    printf("\t0x%02x", ptp_j->fast_read_444_read_instruction);
                } else {
                    printf("\t--");
                }
                printf("\r\nWait states:\t%d\t%d\t%d\t%d\t%d\t%d",
                       ptp_j->fast_read_112_wait_states,
                       ptp_j->fast_read_114_wait_states,
                       ptp_j->fast_read_122_wait_states,
                       ptp_j->fast_read_144_wait_states,
                       ptp_j->fast_read_222_wait_states,
                       ptp_j->fast_read_444_wait_states);
                printf("\r\nMode clocks:\t%d\t%d\t%d\t%d\t%d\t%d",
                       ptp_j->fast_read_112_mode_clocks,
                       ptp_j->fast_read_114_mode_clocks,
                       ptp_j->fast_read_122_mode_clocks,
                       ptp_j->fast_read_144_mode_clocks,
                       ptp_j->fast_read_222_mode_clocks,
                       ptp_j->fast_read_444_mode_clocks);
            ...
            default:
                ...
                printf("Suspend/Resume program: %c\r\nSuspend/Resume erase: %c\r\n",
                       ptp->program_suspend_resume ? 'Y' : '-',
                       ptp->erase_suspend_resume ? 'Y' : '-');
                printf("Wrap Read mode: %c (instruction 0x%02x, length %d)\r\n",
                       ptp->wrap_read_mode ? 'Y' : '-',
                       ptp->wrap_read_instruction,
                       ptp->wrap_read_length);
                printf("Individual block lock: %c (nonvolatile %c, instruction 0x%02x, default %d)\r\n",
                       ptp->individual_block_lock ? 'Y' : '-',
                       ptp->individual_block_lock_volatile ? 'Y' : '-',
                       ptp->individual_block_lock_instruction,
                       ptp->individual_block_lock_volatile_default);
